...
from apps.finance.serializers import PagoProveedorSerializer, RegistrarPagoProveedorRequestSerializer
from apps.finance.exceptions import FinanceException


class PagoProveedorViewSet(ViewSet):
    permission_classes = [IsAuthenticated]

    def _service(self, request):
        return PagoProveedorService(empresa=request.user.empresa, usuario=request.user)
    ...
    def create(self, request):
        ser = RegistrarPagoProveedorRequestSerializer(data=request.data)
        if not ser.is_valid():
